...
const OrderListScreen = () => {
  ...
  const filteredOrders = orders
    ? orders.filter((order) => {
        ...
        const phoneMatch = order.user?.phoneNumber?.toLowerCase().includes(customerFilter.toLowerCase());
                   ...
                   {!userInfo?.isDeliveryAgent && (
                    <td className="px-4 py-3 align-top">
                      <div className="flex flex-col items-start gap-1">
                         ...
                         {order.assignedBy && (
                           <span className="text-[9px] bg-purple-50 text-purple-700 border border-purple-200 px-1.5 py-0.5 rounded mt-0.5 font-bold whitespace-nowrap">
                             {order.assignedBy}
                           </span>
                         )}
                         <button
                           ...
                           className="text-[10px] text-brand-600 hover:text-brand-800 font-bold whitespace-nowrap"
                         >
                      ...
                      (() => {
                        ...
                        return (
                          <div className="flex flex-col gap-2">
                             <span className={`inline-block w-fit rounded-full px-3 py-1.5 text-xs font-semibold whitespace-nowrap ${currentStatusColor}`}>
                               {displayStatus}
                             </span>
                             {userInfo?.isDeliveryAgent && order.returnStatus === 'Approved' && (
                               <button
                                 ...
                                 className="text-[10px] bg-blue-600 text-white px-2 py-1 rounded shadow-sm hover:bg-blue-700 disabled:opacity-50 w-fit whitespace-nowrap"
                               >
                                 Mark Picked Up
                               </button>
                             )}
