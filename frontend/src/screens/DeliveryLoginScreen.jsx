...
const DeliveryLoginScreen = () => {
  ...
  useEffect(() => {
    if (userInfo && isDeliveryAgent(userInfo)) {
      navigate('/delivery/orderlist', { replace: true });
    }
  }, [userInfo, navigate]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="relative w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black text-white mb-1 tracking-tight">Delivery Portal</h1>
          <p className="text-slate-400 text-sm">Buybee Delivery Management</p>
        </div>
          ...
          <form onSubmit={submitHandler} className="space-y-5">
            <div>
              ...
              <input
                ...
                required
                autoFocus
                placeholder="delivery@buybee.com"
                onChange={(e) => setEmail(e.target.value)}
                className="w-full rounded-xl border border-slate-600 bg-slate-700/60 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-600 bg-slate-700/60 px-4 py-2.5 pr-10 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors"
                />
                <button
                  type="button"
            ...
            <button
              ...
              type="submit"
              disabled={isLoading}
              className="w-full py-3 rounded-xl bg-orange-500 hover:bg-orange-400 disabled:opacity-50 text-white font-bold text-sm shadow-md shadow-orange-900/30 transition-all duration-150 flex items-center justify-center gap-2 mt-1"
            >
              {isLoading ? <Loader /> : 'Sign In to Delivery Portal'}
            </button>
          </form>
        </div>
